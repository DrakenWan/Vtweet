...
<?php
        ...
        $query1 = "SELECT complain_no, comp, dept, room_no FROM hostel where dept='Room' and block = 'G'";
        $row1 = mysqli_query($conn,$query1);
        while($row = mysqli_fetch_assoc($row1)) {
        echo $row["dept"]."   :  ".$row["room_no"]."   :  ".$row["comp"]."   =>  ".$row["complain_no"]."<br>";
        }
        $query3 = "SELECT complain_no, comp, dept, room_no FROM hostel where dept='Washroom' and block = 'G'";
        $row3 = mysqli_query($conn,$query3);
        while($row = mysqli_fetch_assoc($row3)) {
        echo $row["dept"]."   :  ".$row["room_no"]."   :  ".$row["comp"]."   =>  ".$row["complain_no"]."<br>";
        }

        $query4 = "SELECT complain_no, comp, dept, room_no FROM hostel where dept='Wifi' and block = 'G'";
        $row4 = mysqli_query($conn,$query4);
        while($row = mysqli_fetch_assoc($row4)) {
        echo $row["dept"]."   :  ".$row["room_no"]."   :  ".$row["comp"]."   =>  ".$row["complain_no"]."<br>";
        }

        $query5 = "SELECT complain_no, comp, dept, room_no FROM hostel where dept='Mess' and block = 'G'";
